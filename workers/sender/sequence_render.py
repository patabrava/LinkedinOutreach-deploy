...
Canonical names: first_name, last_name, full_name, company_name, salutation.
...
_CANONICAL = {"first_name", "last_name", "full_name", "company_name", "salutation"}
_ALIASES = {"VORNAME": "first_name", "NACHNAME": "last_name"}
_VERIFIED_SALUTATIONS = {"Frau", "Herr"}
...
def _resolve(name: str, lead: Dict[str, Any]) -> Optional[str]:
    ...
    if canonical == "salutation":
        ...
        return salutation if salutation in _VERIFIED_SALUTATIONS else ""
...
def render(template: str, lead: Dict[str, Any]) -> str:
    if re.search(r"\{\{\s*salutation\s*\}\}|\{\s*salutation\s*\}|\[\s*salutation\s*\]", template or ""):
        salutation = _resolve("salutation", lead)
        if salutation not in _VERIFIED_SALUTATIONS:
