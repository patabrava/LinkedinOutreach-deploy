-- Restore the approved DEGURA LinkedIn Version 2 copy after the historical
-- seed paraphrased and omitted parts of several messages. Placeholder names
-- are normalized to the runtime contract; the explicit 30-minute decision is
-- retained where the document still says 20 minutes.

UPDATE outreach_sequence_variants AS variant
SET connect_note = copy.connect_note,
    first_message = copy.first_message,
    second_message = copy.second_message,
    third_message = copy.third_message,
    asset_followup_1 = copy.asset_followup_1,
    asset_followup_2 = copy.asset_followup_2,
    updated_at = now()
FROM outreach_sequences AS sequence,
(VALUES
  ('DEGURA_A', 1,
$copy$Hallo {{first_name}}, seit dem Bericht der Rentenkommission fragen Mitarbeitende bei HR nach ihrer Rente. Ich sehe gerade, inwiefern Unternehmen wie {{company_name}} darauf antworten. Würde mich gern mit dir vernetzen.$copy$,
...
Ansonten, wünsche ich Dir weiterhin viel Erfolg und einen schönen Tag.$copy$,
...
Der Leitfaden zeigt, wie bAV grundsätzlich funktioniert. Was er nicht zeigt: was bei {{company_name}} tatsächlich rauskommt, mit euren Gehältern, euren Verträgen und euren Systemen. Das können wir gemeinsam in 30 Minuten durchrechnen - unverbindlich.

Hier ist der Meetinlink von unserem bAV Experten: https://calendly.com/toby-weber-degura/videotelefonat-mit-toby-30min
...
Hast du diese oder nächste Woche für einen kurzen Call? Herr Weber, unser bAV Experte, kann Dir in 30 Minuten unverbindlich zeigen, was sich für deine Belegschaft rechnet: https://calendly.com/toby-weber-degura/videotelefonat-mit-toby-30min$copy$,
...
Die bAV muss kein Kostenblock sein. Wenn sie clever aufgesetzt ist, ist sie ein starkes strategisches Instrument.

Hättest Du Lust dich dazu unverbindlich auszutauschen?$copy$,
$copy$Noch eine letzte Frage von mir, dann lasse ich dich in Ruhe.
...
Wenn das gerade nicht passt, ist das auch eine Antwort. Dann höre ich auf.$copy$),
  ...
  ('DEGURA_B', 2,
$copy$Hallo {{first_name}}, ich arbeite mit HR-Teams in Unternehmen wie {{company_name}} am Thema betriebliche Altersvorsorge. Würde mich gern vernetzen, wenn Du Lust hast, dich auszutauschen.$copy$,
$copy$Danke fürs Vernetzen, {{first_name}}.

Etwas, das ich in vielen HR Gesprächen höre: Die bAV ist eingerichtet, sie erfüllt die Pflicht aus dem BetrAVG, und trotzdem nutzt sie kaum jemand. Meistens nicht, weil das Interesse fehlt, sondern weil sie nicht richtig kommuniziert wird. Wie macht ihr es aktuell bei {{company_name}}?
...
Wir haben zusammengefasst, wie {{company_name}} die bAV als ein strategisches Benefit nutzen kann, welches echte Wirkung zeigt und Talente langfristig bindet.

Soll ich dir den Leitfaden schicken?$copy$,
$copy$Hier ist ein letzter Impuls von mir, dann lasse ich dich in Ruhe.

Falls das Thema irgendwann hochkommt, weil jemand aus dem Team fragt oder ihr euch mit strategischen Benefits für {{company_name}} beschäftigt: Hier ist unser Leitfaden.

https://www.degura.de/bav-leitfaden-confirmation
...
Wenn das derzeit nicht passt, ist das ebenfalls eine Antwort.$copy$)
  ...
  campaign_key, variant_key, connect_note, first_message, second_message,
  third_message, asset_followup_1, asset_followup_2
)
WHERE sequence.id = variant.sequence_id
  AND sequence.campaign_key = copy.campaign_key
  AND variant.variant_key = copy.variant_key;

DO $$
BEGIN
  IF (SELECT count(*) FROM outreach_sequence_variants variant
    ...
    RAISE EXCEPTION 'DEGURA_COPY_SYNC_FAILED: expected six active variants';
  END IF;
END $$;
